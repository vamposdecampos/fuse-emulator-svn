...
int
if1_fdc_eject( if1_drive_number which )
{
  upd_fdc_drive *d;
  ...
  if( d->disk.dirty ) {
    ui_confirm_save_t confirm = ui_confirm_save( "Disk in drive %d has been modified.\n"
...
int
if1_fdc_save( if1_drive_number which, int saveas )
{
  upd_fdc_drive *d;
...
int
if1_fdc_flip( if1_drive_number which, int flip )
{
  upd_fdc_drive *d;
...
int
if1_fdc_writeprotect( if1_drive_number which, int wrprot )
{
  upd_fdc_drive *d;
...
int
if1_fdc_disk_write( if1_drive_number which, const char *filename )
{
