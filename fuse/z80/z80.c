...
z80_interrupt_event_fn( libspectrum_dword tstates, int type, void *user_data )
{
  ...
  if (hack_pending_vector >= 0) {
    z80_interrupt_vector( hack_pending_vector );
    hack_pending_vector = -1;
    return;
  }
...
z80_interrupt_vector( libspectrum_byte vector )
{
  ...
  fprintf(stderr, "%s: IFF1=%d; halted=%d; PC=0x%04x\n", __func__, IFF1, z80.halted, PC);
  if( !IFF1 && vector != 0xff )
    hack_pending_vector = vector;
  ...
  if( IFF1 &&
    ...
    if( tstates == z80.interrupts_enabled_at ) {
      fprintf(stderr, "############### retrig! ######################\n");
      event_add_with_data( tstates + 1, z80_interrupt_event, (void *) vector );
      return 0;
    }
...
z80_interrupt( void )
{
  if( tstates >= machine_current->timings.interrupt_length )
    return 0;			/* Did not accept an interrupt */
