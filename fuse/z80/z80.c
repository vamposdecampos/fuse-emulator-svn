...
z80_interrupt_vector( libspectrum_byte vector )
{
  ...
  //if( vector != 0xff )
    fprintf(stderr, "%s: vector 0x%02x IFF1=%d; halted=%d; PC=0x%04x\n", __func__, vector, IFF1, z80.halted, PC);

  if( IFF1 &&
      !scld_last_dec.name.intdisable ) {

    if( tstates == z80.interrupts_enabled_at ) {
      fprintf(stderr, "too soon, would retrig\n");
      return 0;
    }
      ...
      case 2: 
	{
	  ...
	  fprintf(stderr, "%s: interrupt PC %04x (I=%02x vector %02x)\n", __func__, PC, I, vector);
	  tstates += 7;
	  break;
	}
      default:
